...
function Chunk.Load(chunkPositions : {Vector3})
	...
	for i, chunkPosition in pairs(chunkPositions) do
		assert(typeof(chunkPosition) == "Vector3", "[[Voxel Sea]][Chunk.Load] Argument #1: Unexpected value type \"" .. typeof(chunkPosition) .. "\" found in Vector3 array. Argument #1 must be an array containing only Vector3s.")
		...
		if Chunk.GetChunkFromPos(chunkPosition) then
			warn("The chunk at chunk position " .. tostring(chunkPosition) .. " is already loaded.")
			continue
		end
...
function Chunk.SetLoadDataCallback(callback)
...
function Chunk.GetChunkFromVoxelPos(voxel_pos : Vector3) : {} | nil
	...
	local chunkPos = Chunk.GetChunkPosFromVoxelPos(voxel_pos)
	local chunk = Chunk.GetChunkFromPos(chunkPos)
	if not chunk then
		return Chunk.Load({chunkPos})[1]
	end
end
...
function Chunk:CompactAndRender()
	...
	while true do
		...
		if voxel_part.Parent ~= replicator.VoxelSeaBuilderPluginObjectsFolder then
			voxel_part.Parent = replicator.VoxelSeaBuilderPluginObjectsFolder
		end
