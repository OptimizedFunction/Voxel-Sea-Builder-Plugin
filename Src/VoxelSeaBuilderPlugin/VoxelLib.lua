...
function Voxel.new(materialCode : number?, transparency : number?, reflectance : number?, rbxMaterial : Enum.Material?, color : Color3?) : {any}
	assert(materialCode == nil or typeof(materialCode) == "number", "[[Voxaria]][Voxel.new] Argument #1 must be either a number or nil")
	...
	for x = init_voxel_pos.X, final_voxel_pos.X, voxel_size do
		for y = init_voxel_pos.Y, final_voxel_pos.Y, voxel_size do
			for z = init_voxel_pos.Z, final_voxel_pos.Z, voxel_size do
				...
				local chunkPosition = ChunkClass.GetChunkPosFromVoxelPos(Vector3.new(x,y,z))
				local chunk = ChunkClass.GetChunkFromPos(chunkPosition)

				if not chunk then 
					chunk = ChunkClass.Load({chunkPosition})[1]
				end

				local index = Voxel.GetIndex(Utility.World_To_Relative(chunk:GetInitCorner(), x, y, z))
